"""
words.py — CRUD de palabras + reset
"""
import re
...
def words():
    ...
    if method == "POST" and action == "validate":
        ...
        warning = f"La palabra {invalid_labels[0]} parece incorrecta" if len(invalid_labels) == 1 else f"Las palabras {', '.join(invalid_labels)} parecen incorrectas"
    ...
    if method == "POST" and action == "add_synonym":
        ...
        if not st:
            return err("Grupo no encontrado", 404)
        existing = db_fetchone("SELECT id FROM words WHERE group_id = %s AND LOWER(english) = LOWER(%s)", (group_id, word))
        if existing:
...
def words():
    ...
    if method == "GET" and action == "list":
        ...
        rows = db_fetchall(
            """SELECT g.id, g.spanish, g.created_at, g.example_sentence,
               ...
               FROM word_groups g
               JOIN words w ON w.group_id = g.id
               WHERE g.user_id = %s AND DATE(g.created_at) = %s
               GROUP BY g.id
               ORDER BY g.id DESC""",
            (uid, d),
        )
    ...
    if method == "GET" and action == "week":
        ...
        rows = db_fetchall(
            """SELECT g.id, g.spanish, g.created_at, g.example_sentence,
               ...
               FROM word_groups g
               JOIN words w ON w.group_id = g.id
               WHERE g.user_id = %s AND DATE(g.created_at) BETWEEN %s AND %s
               GROUP BY g.id
    ...
    if method == "GET" and action == "weekly_stats":
        rows = db_fetchall(
            """SELECT DATE_SUB(created_at, INTERVAL WEEKDAY(created_at) DAY) AS week_start,
                      COUNT(*) AS total_groups,
                      COUNT(DISTINCT created_at) AS days_active
               FROM word_groups WHERE user_id = %s
               GROUP BY week_start
        ...
        for r in rows:
            r["week_start"] = str(r["week_start"]) if r.get("week_start") else None
        return ok(rows)
    ...
    if method == "GET" and action == "search":
        ...
        like = f"%{q}%"
        if lang == "es":
            where = "g.spanish LIKE %s"
            params = (uid, like)
        elif lang == "en":
            where = "w.english LIKE %s"
            params = (uid, like)
        else:
            where = "(g.spanish LIKE %s OR w.english LIKE %s)"
            params = (uid, like, like)
        rows = db_fetchall(
            f"""SELECT DISTINCT g.id, g.spanish, g.created_at,
                       GROUP_CONCAT(w.english ORDER BY w.id SEPARATOR '||') AS english_words
    ...
    if method == "GET" and action == "distractors":
        ...
        rows = db_fetchall(
            """SELECT g.id, g.spanish,
               ...
               WHERE g.user_id = %s AND g.id != %s
               GROUP BY g.id
               ORDER BY ABS(DATEDIFF(g.created_at, %s)), RAND()
               LIMIT 3""",
            (uid, gid, d),
        )
        for r in rows:
    ...
    if method == "POST" and action == "validate":
        b = body()
        spanish = b.get("spanish", "").strip()
        english = [e.strip() for e in b.get("english", []) if str(e).strip()]
        if not spanish or not english:
            return err("Faltan campos")
        ...
        invalid_words = [r for r in word_results if not r["valid"]]
        invalid_labels = [f'"{r["word"]}"' for r in invalid_words]
        if len(invalid_labels) == 1:
            warning = f"La palabra {invalid_labels[0]} parece incorrecta"
        else:
            warning = f"Las palabras {', '.join(invalid_labels)} parecen incorrectas"
        return ok({"valid": False, "warning": warning, "suggestion": "", "word_results": word_results})
    ...
    if method == "POST" and action == "add":
        ...
        existing = db_fetchone(
            "SELECT id, created_at FROM word_groups WHERE user_id = %s AND spanish = %s LIMIT 1",
            (uid, spanish),
        )
        if existing:
            return ok({"duplicate": True, "group_id": int(existing["id"])})
        ...
        conn = g.db if hasattr(g, "db") else None
        ...
        try:
            ...
            with conn.cursor() as cur:
                cur.execute(
                    "INSERT INTO word_groups (user_id, spanish, created_at, category) VALUES (%s, %s, %s, %s)",
                    (uid, spanish, today, category),
                )
                gid = cur.lastrowid
                for en in english:
            ...
            # Generate example sentence async (non-blocking)
            try:
                prompt = (
                    f'You are an English tutor. Create ONE short natural English sentence (10-15 words) using the word "{english[0]["word"]}". '
                    f'The sentence should clearly show the word\'s meaning (Spanish: "{spanish}"). '
                    'Reply ONLY with a JSON object, no markdown: {"sentence":"the English sentence","translation":"Spanish translation of the sentence"}'
                )
                raw = groq_call(prompt, 100)
                parsed = parse_groq_json(raw)
                if parsed and parsed.get("sentence"):
                    ...
                    if parsed.get("translation"):
                        example += " — " + parsed["translation"]
                    db_update(
                        "UPDATE word_groups SET example_sentence = %s WHERE id = %s",
                        (example, gid),
                    )
            except Exception:
                pass
    ...
    if method == "POST" and action == "set_word_diff":
        ...
        word_rows = db_fetchall(
            "SELECT id FROM words WHERE group_id = %s ORDER BY id ASC", (gid,)
        )
        if word_index >= len(word_rows):
            return err("Palabra no encontrada", 404)
        wid = word_rows[word_index]["id"]
        db_update("UPDATE words SET is_hard = %s WHERE id = %s", (1 if difficulty == "hard" else 0, wid))
        return ok({"word_id": wid, "difficulty": difficulty, "is_hard": 1 if difficulty == "hard" else 0})
    ...
    if method == "DELETE" and action == "delete":
        ...
        if not gid:
            return err("ID requerido")
        rows_affected = db_update(
            "DELETE FROM word_groups WHERE id = %s AND user_id = %s", (gid, uid)
        )
        if rows_affected == 0:
            return err("Grupo no encontrado", 404)
        return ok(None)
    ...
    if method == "POST" and action == "reset":
        ...
        if scope == "all":
            ...
            gids = [r["id"] for r in db_fetchall(
                "SELECT id FROM word_groups WHERE user_id = %s AND created_at = %s",
                (uid, value),
            )]
            if gids:
                ph = ",".join(["%s"] * len(gids))
            ...
            return err("scope inválido. Usa: all | week (con value=YYYY-MM-DD) | date (con value=YYYY-MM-DD)")
    ...
    if method == "POST" and action == "add_synonym":
        ...
        if not group_id or not word:
            return err("Faltan campos")
        st = db_fetchone(
            "SELECT id FROM word_groups WHERE id = %s AND user_id = %s", (group_id, uid)
        )
        if not st:
            return err("Grupo no encontrado", 404)
        existing = db_fetchone(
        ...
        if existing:
            return ok({"added": False, "message": "Ya existe"})
        db_insert("INSERT INTO words (group_id, english, is_hard) VALUES (%s, %s, 0)", (group_id, word))
        return ok({"added": True})
