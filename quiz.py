"""
quiz.py — Quiz semanal con priorización SRS
"""
import random
...
def srs_update_quiz(uid: int, group_id: int, quality: int):
    try:
        ...
        mastered = 1 if (interval >= 21 and quality >= 4) else 0
        db_update(
...
def groq_simple(prompt: str) -> str | None:
    return groq_call(prompt, 80)
...
def quiz():
    ...
    if method == "GET" and action == "questions":
        ...
        for r in rows:
            ...
            if nr <= today and not r["mastered"]:
                score += 100
            score += (2.5 - ef) * 30
        ...
        for r in rows:
            ...
            r["question"] = (
                r["english_words"][random.randint(0, len(r["english_words"]) - 1)]
                if r["direction"] == "en_es"
                else r["spanish"]
            )
            ...
            reps = int(r["repetitions"])
            if mastered:
                r["srs_level"] = "dominada"
            elif reps == 0:
                r["srs_level"] = "nueva"
            elif interval <= 3:
                r["srs_level"] = "aprendiendo"
            elif interval <= 14:
                r["srs_level"] = "repasando"
            else:
                r["srs_level"] = "consolidada"
            for k in ("_score", "easiness", "repetitions", "interval", "next_review", "mastered"):
                r.pop(k, None)
    ...
    if method == "GET" and action == "check_done":
        ...
        if ws_str:
            ...
            today_d = date.today()
            ws = str(today_d - timedelta(days=today_d.weekday()))
        row = db_fetchone(
            "SELECT id, score, total FROM weekly_tests WHERE user_id = %s AND week_start = %s ORDER BY id DESC LIMIT 1",
            (uid, ws),
        )
    ...
    if method == "POST" and action == "submit":
        b = body()
        today_d = date.today()
        default_ws = str(today_d - timedelta(days=today_d.weekday()))
        week_start = b.get("week_start", default_ws)
        answers = b.get("answers", [])
        duration = int(b.get("duration_secs", 0))
        if not answers:
            return err("Sin respuestas")
        ...
        for ans in answers:
            ...
            if answer:
                if direction == "es_en":
                    prompt = f'Palabra español: "{spanish}". Respuesta: "{answer}". Correctas: {", ".join(english_list)}. ¿Correcto? Acepta sinónimos. SOLO JSON: {{"correct":true/false,"feedback":"max 15 palabras"}}'
                else:
                ...
                if not feedback:
                    if direction == "es_en":
                        correct = answer.lower() in [e.lower() for e in english_list]
                    else:
                        correct = answer.lower() == spanish.lower()
            ...
            db_insert(
                ...
                (uid, gid, direction, answer, int(correct), f"Quiz: {feedback}"),
            )
        ...
        db_insert(
            "INSERT INTO weekly_tests (user_id, week_start, score, total) VALUES (%s, %s, %s, %s)",
            (uid, week_start, score, len(answers)),
        )
        try:
            db_insert(
                "INSERT INTO session_history (user_id, session_date, practice_mode, total, correct, duration_secs) VALUES (%s, %s, 'quiz', %s, %s, %s)",
                (uid, today, len(answers), score, duration or None),
            )
        except Exception:
            pass
    ...
    if method == "GET" and action == "history":
        rows = db_fetchall(
            "SELECT * FROM weekly_tests WHERE user_id = %s ORDER BY created_at DESC LIMIT 10",
            (uid,),
        )
        for r in rows:
            if r.get("week_start"):
    ...
    if method == "GET" and action == "questions_n":
        ...
        if src == "due":
            sql = base + " AND (s.mastered = 0 OR s.mastered IS NULL) AND (s.next_review <= %s OR s.next_review IS NULL)"
        ...
        sql += f" GROUP BY g.id ORDER BY RAND() LIMIT {n}"
